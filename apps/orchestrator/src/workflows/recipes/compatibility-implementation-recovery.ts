...
export class CompatibilityImplementationRecovery {
  ...
  async recover(feature: WorkItemCard, project: StoredProject, metadata: Metadata): Promise<WorkItemCard | null> {
    const kind = compatibilityRecoveryKind(feature, this.dependencies.validateImplementationArtifacts(feature.folderPath));
    ...
    await this.dependencies.metadata.start({ ...metadata, currentNodeId: "implementation-recovery", currentStep: "Recovering implementation lifecycle state",
      summary: "Repairing stale FeatureTasks status; implementation is paused until filesystem validation passes." });
    ...
    try {
      ...
      if (!await this.dependencies.isWorkflowActive(metadata)) return null;
      if (repaired.stateFolder !== "03_IN_PROGRESS" || repaired.folderPath !== feature.folderPath) {
        return this.block(feature, metadata, "RECOVERY_REJECTED: The unique In Progress location changed during header repair.");
      }
