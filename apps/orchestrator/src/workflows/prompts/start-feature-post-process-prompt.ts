...
export function buildStartFeaturePostProcessPrompt(
  ...
  return [
    ...
    `- Verify the explicitly declared planning responsibility includes \`${options.featurePlanningArtifactFileName}\` as the durable cross-phase planning handoff.`,
    "- If that planning artifact requirement is missing, add it to the phase that owns planning and FeatureTasks.md; do not select a phase by number or title without changing feature scope.",
