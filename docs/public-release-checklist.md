...
- [x] Prepare and audit a clean local release-candidate tree outside the
  working repository.
- [ ] Review and approve the destructive cutover gates in
  [`public-cutover-plan.md`](public-cutover-plan.md).
...
- [ ] Replace the remote history with the clean root. No credential rotation is
...
The 2026-09-05 Gitleaks 8.30.1 scan inspected every local ref. Its 18 findings
...
inline annotations. Repeat the scan immediately before cutover.
...
- [x] Reproduce installation, type checking, tests, and the production build
  from the clean candidate tree on Linux.
